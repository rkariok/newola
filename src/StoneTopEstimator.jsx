...
export default function StoneTopEstimator() {
  ...
  // Settings - Initialize with NO BUFFER and NO KERF as defaults
  ...
  // Handle bulk products from AI parsing
  ...
  const calculateAll = () => {
    ...
    // Pass settings directly - they already have the correct defaults
    if (settings.multiProductOptimization) {
      // Use multi-type optimization
      optimizationResults = optimizeMultiTypeLayout(productsWithStones, stoneOptions, settings);
      results = applyMultiTypeOptimization(productsWithStones, optimizationResults, stoneOptions, settings);
  ...
  const loadQuote = (quote) => {
    ...
    // When loading a quote, respect the saved settings
